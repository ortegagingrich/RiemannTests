...
!     real(kind=8) :: bpadq(meqn,1-mbc:maxm + mbc)
    ...
    !Common block for recording how many roe solves and how many full solves were done
    integer totalcount,roecount,fullcount
    common /counttype/ totalcount,roecount,fullcount
    !output the number of roe solves for the previous grid
    print *,'Roe solves: ',roecount,' out of ',totalcount,' total solves.',&
    		'Fraction:',roecount/(1.d0+totalcount)
    !reset counts for the present grid
    totalcount=0
    roecount=0
    fullcount=0
