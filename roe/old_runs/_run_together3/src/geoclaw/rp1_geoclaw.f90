...
!-----------------------Initializing-----------------------------------
         ...
         if(hRr.ge.deeptol.and.hLr.ge.deeptol) then
         	...
         	uhat=(sqrt(hLr)*uLr + sqrt(hRr)*uRr)/(sqrt(hRr)+sqrt(hLr))
         	chat=sqrt(g*0.5d0*(hRr+hLr))
         	sRoe1=uhat-chat ! Roe wave speed 1 wave
         	sRoe2=uhat+chat ! Roe wave speed 2 wave
         	deltah=hRr-hLr
         	deltahu=huRr-huLr
         	!solve linear system
         	a1 = 0.5d0*(-deltahu + sRoe2 * deltah)/chat
       		a2 = 0.5d0*( deltahu - sRoe1 * deltah)/chat
       		!set wave speeds
       		s(3,i)=sRoe1
       		s(4,i)=sRoe2
       		!compute fwaves
       		fwave(3,3,i)=sRoe1*a1
       		fwave(4,3,i)=sRoe1*a1*(uhat-chat)
       		fwave(3,4,i)=sRoe2*a2
       		fwave(4,4,i)=sRoe2*a2*(uhat+chat)
       		fwave(1,3,i)=0
       		fwave(1,4,i)=0
       		fwave(2,3,i)=0
       		fwave(2,4,i)=0
