...
subroutine shallow_full(i,info,hLin,hRin,huLin,huRin,bLin,bRin,swfull,fwfull)
	...
	double precision, intent(out) :: info !information to go into aux array
	...
	if(max(abs(fw(1,2)),abs(fw(2,2))).gt.(1d-2)) then
		!set info to indicate that there is a large corrector wave
		info=1.d0
		!print *,"Corrector Wave: [",fw(1,2),",",fw(2,2),"], Speed: ",sw(2)
		!print *,"Big corrector wave at i=",i
	endif
