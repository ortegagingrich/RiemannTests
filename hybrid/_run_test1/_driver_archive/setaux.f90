subroutine setaux(mbc,mx,xlower,dx,maux,aux)
    ...
    do i=1-mbc,mx+mbc
         ...
         aux(1,i)=-0.01d0*0+0.5d0*exp(-0.005d0*(xcell-35)**2)&
         	&+2.0d0*exp(-0.005*(xcell-75)**2)
         !1.8d0*exp(-10.d-2*xcell**2) + .5d0*1.0d0*exp(-.5d-2*(xcell-0.5d2)**2) &
         !   &  + 1.8d0*exp(-10.d-2*(xcell-1.d2)**2)
         
         !set second (debug) aux to zero
         aux(2,i)=0.d0
